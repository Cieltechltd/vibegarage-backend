...
def listener_dashboard(
    ...
    total_follows = db.query(Follow).filter(Follow.follower_id == current_user.id).count()
    total_plays = db.query(Play).filter(Play.user_id == current_user.id).count()
...
def recommendations(
    ...
    artist_ids = list({play.track.artist_id for play in recent_plays})
    if not artist_ids:
        return {"results": []}

    tracks = db.query(Track).filter(Track.artist_id.in_(artist_ids)).limit(20).all()
    return {
        "count": len(tracks),
        "results": [
            TrackPublic(
